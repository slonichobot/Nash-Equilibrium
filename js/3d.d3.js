(function (global, factory) {
	...
	function centroid(polygon){
		let _x = 0, _y = 0, _z = 0, _n = polygon.length;
	...
	function rotateRzRyRx(po, angles){
		...
		let rc = angles.rotateCenter;

		po.x -= rc[0];
		po.y -= rc[1];
		po.z -= rc[2];

		let rz = rotateZ(po, angles.z);
		let ry = rotateY(rz, angles.y);
		let rx = rotateX(ry, angles.x);
	...
	function cube(cubes, options, point$$1, angles){
		for (let i = cubes.length - 1; i >= 0; i--) {
			...
			let front  = [v1, v2, v3, v4];
			let back   = [v8, v7, v6, v5];
			let left   = [v5, v6, v2, v1];
			let right  = [v4, v3, v7, v8];
			let top    = [v5, v1, v4, v8];
			let bottom = [v2, v6, v7, v3];
	...
	function gridPlane(grid, options, point$$1, angles){
		...
		let points = point(grid, options, point$$1, angles);
		let cnt    = 0, planes = [];
		let numPts = options.row;
		let numRow = points.length/numPts;

		for (let i = numRow - 1; i > 0; i--) {
			for (let j = numPts - 1; j > 0; j--) {

				let p1 = j + i * numPts, p4 = p1 - 1, p2 = p4 - numPts + 1, p3 = p2 - 1;
				let pl = [points[p1], points[p2], points[p3], points[p4]];
	...
	function lineStrip(lineStrip, options, point, angles){
		...
		for (let i = lineStrip.length - 1; i >= 0; i--) {
			...
			let l = lineStrip[i], m = l.length/2, t = parseInt(m);
	...
	function polygon(polygons, options, point, angles){
		...
		for (let i = polygons.length - 1; i >= 0; i--) {
			...
			for (let j = 0; j < poly.length; j++) {
				poly[j].rotated = rotateRzRyRx({x: point.x(poly[j]), y: point.y(poly[j]), z: point.z(poly[j])}, angles);
				poly[j].projected = options.project(poly[j].rotated, options);
			}
	...
	function drawLineStrip(lineStrip){
		let lastPoint = lineStrip[lineStrip.length - 1];
		let path = 'M' + lastPoint.projected.x + ',' + lastPoint.projected.y;
		for (let i = lineStrip.length - 2; i >= 0; i--) {
			let p = lineStrip[i].projected;
			path += 'L' + p.x + ',' + p.y;
		}
		return path;
	}
	...
	function drawPolygon(d){
		return 'M' + (d.map(p =>  p.projected.x + ',' + p.projected.y ).join('L')) + 'Z';
	}
	...
	let _3d = function() {
		...
		_3d.sort = function(a, b){
			let _a = a.centroid.z, _b = b.centroid.z;
